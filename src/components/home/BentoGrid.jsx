import React, { useState, useEffect } from 'react';
import { Link } from 'react-router-dom';
import { Shield, Zap, ArrowUpRight, Clock } from 'lucide-react';
import { collection, query, where, getDocs, doc, getDoc, orderBy, limit } from 'firebase/firestore';
import { db } from '../../config/firebase';
import { useLanguage } from '../../i18n/LanguageContext';

const BentoGrid = () => {
    const { t, lang } = useLanguage();
    const [articles, setArticles] = useState([]);
    const [riskCard, setRiskCard] = useState(null);
    const [auCard, setAuCard] = useState(null);

    useEffect(() => {
        const fetchAll = async () => {
            try {
                // Articles
                const q = query(
                    collection(db, 'articles'),
                    where('status', '==', 'published'),
                    orderBy('createdAt', 'desc'),
                    limit(20)
                );
                const snapshot = await getDocs(q);
                const fetched = snapshot.docs.map(d => ({ id: d.id, ...d.data() }));
                setArticles(fetched);

                // Bento cards
                const [riskSnap, auSnap] = await Promise.all([
                    getDoc(doc(db, 'bentoCards', 'bento-risk')),
                    getDoc(doc(db, 'bentoCards', 'bento-australia')),
                ]);
                if (riskSnap.exists()) setRiskCard(riskSnap.data());
                if (auSnap.exists()) setAuCard(auSnap.data());
            } catch (err) {
                console.error('Error fetching bento data:', err);
            }
        };
        fetchAll();
    }, []);

    const categoryColors = {
        hukuk: '#ccff00',
        egitim: '#00d4ff',
        sosyal: '#ff6b6b',
        'program-turleri': '#a78bfa',
    };

    const CAT_MAP = { hukuk: t('cat_hukuk'), egitim: t('cat_egitim'), sosyal: t('cat_sosyal'), 'program-turleri': t('cat_program') };

    const formatDate = (timestamp) => {
        if (!timestamp) return '';
        const date = timestamp.toDate ? timestamp.toDate() : new Date(timestamp);
        const now = new Date();
        const diff = now - date;
        const hours = Math.floor(diff / (1000 * 60 * 60));
        const days = Math.floor(diff / (1000 * 60 * 60 * 24));
        if (hours < 1) return t('time_just_now');
        if (hours < 24) return `${hours}${t('time_h_ago')}`;
        if (days < 7) return `${days}${t('time_d_ago')}`;
        return date.toLocaleDateString(lang === 'tr' ? 'tr-TR' : 'en-AU', { day: 'numeric', month: 'short' });
    };

    return (
        <section className="max-w-[1600px] mx-auto px-6 py-24">
            <div className="flex items-end justify-between mb-16">
                <div>
                    <h2 className="text-[10px] font-black tracking-[0.4em] text-[#ccff00] uppercase mb-4">{t('section_flow')}</h2>
                    <h3 className="text-4xl md:text-6xl font-black uppercase tracking-tighter">{t('section_analyses')}</h3>
                </div>
                <div className="text-right hidden md:block">
                    <p className="text-[10px] text-white/40 font-bold uppercase tracking-widest leading-loose">
                        {t('data_source')}<br />
                        {t('data_update')}: {new Date().toLocaleTimeString()}
                    </p>
                </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                {/* Bento Item 1 — Risk Analysis (editable via admin) */}
                <div className="md:col-span-2 md:row-span-2 bg-[#111] p-10 border border-white/5 flex flex-col justify-between min-h-[400px]">
                    <div>
                        <div className="flex justify-between items-start mb-12">
                            <Shield className="text-[#ccff00]" size={40} />
                            <span className="text-[10px] font-bold text-white/40">
                                {riskCard?.safeData || t('safe_data')}
                            </span>
                        </div>
                        <h4 className="text-3xl font-black uppercase leading-tight italic">
                            {riskCard?.title || t('bento_title')}
                        </h4>
                    </div>
                    <div className="mt-8">
                        <p className="text-sm text-white/40 max-w-[250px] uppercase font-bold tracking-tight">
                            {riskCard?.desc || t('bento_desc')}
                        </p>
                    </div>
                </div>

                {/* Bento Item 2 — Australia stats (editable via admin) */}
                <div className="md:col-span-2 bg-[#ccff00] text-black p-10 flex flex-col justify-center relative overflow-hidden">
                    <Zap className="absolute right-[-20px] top-[-20px] w-48 h-48 opacity-10 rotate-12" />
                    <h4 className="text-4xl font-black italic tracking-tighter uppercase mb-4">
                        {auCard?.auTitle || t('australia')}
                    </h4>
                    <div className="flex gap-8 mt-4">
                        <div>
                            <div className="text-2xl font-black">{auCard?.stat1Value || '%98'}</div>
                            <div className="text-[10px] font-bold uppercase tracking-widest">{auCard?.stat1Label || t('data_accuracy')}</div>
                        </div>
                        <div>
                            <div className="text-2xl font-black">{auCard?.stat2Value || '7/24'}</div>
                            <div className="text-[10px] font-bold uppercase tracking-widest">{auCard?.stat2Label || t('legal_tracking')}</div>
                        </div>
                    </div>
                </div>

                {/* Dynamic Article Blocks from Firestore */}
                {articles.length > 0 ? (
                    articles.map((article) => (
                        <Link
                            to={`/makale/${article.slug}`}
                            key={article.id}
                            className="bg-[#111] p-8 border border-white/5 hover:border-white/20 transition-all cursor-pointer flex flex-col justify-between group"
                            aria-label={`${(lang === 'en' ? article.title_en || article.title : article.title_tr || article.title)} — ${t('article_read')}`}
                        >
                            <div className="flex justify-between text-[10px] font-bold text-white/40 mb-8 uppercase tracking-widest">
                                <span style={{ color: categoryColors[article.category] || '#ccff00' }}>
                                    {CAT_MAP[article.category] || article.category?.toUpperCase()}
                                </span>
                                <span className="flex items-center gap-1">
                                    <Clock size={10} />
                                    {formatDate(article.createdAt)}
                                </span>
                            </div>
                            <h4 className="text-xl font-black uppercase leading-none group-hover:text-[#ccff00] transition-colors">
                                {lang === 'en' ? (article.title_en || article.title) : (article.title_tr || article.title)}
                            </h4>
                        </Link>
                    ))
                ) : (
                    <div className="md:col-span-2 bg-[#111] p-8 border border-white/5 flex items-center justify-center">
                        <p className="text-white/20 text-sm font-bold uppercase tracking-widest">
                            {t('page_coming_soon')}
                        </p>
                    </div>
                )}
            </div>
        </section>
    );
};

export default BentoGrid;
